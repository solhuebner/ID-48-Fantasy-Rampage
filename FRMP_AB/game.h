...
void display_card (char x, char y, char card, char modifier) {
  ...
  if (game_mode == GAME_MODE_ADVANCED) {
    ...
    switch (curr_modifier) {
      case 0:
        break;
      case 1:
        sprites.drawOverwrite(curr_x + 56, curr_y, cardSprites, IMG_BONUS);
        break;
      case -1:
        sprites.drawOverwrite(curr_x + 56, curr_y, cardSprites, IMG_MINUS_1);
        break;
      case -2:
        sprites.drawOverwrite(curr_x + 56, curr_y, cardSprites, IMG_MINUS_2);
        break;
  ...
  sprites.drawOverwrite(curr_x + 4, curr_y + 4, cardSprites, inf.suit + 10);
  sprites.drawOverwrite(curr_x += 16, curr_y, cardSprites, 1);
  sprites.drawOverwrite(curr_x += 8, curr_y, cardSprites, 1);
  sprites.drawOverwrite(curr_x += 8, curr_y, cardSprites, 1);
  sprites.drawPlusMask(curr_x += 8, curr_y, cardCorners_plus_mask, 1);
  sprites.drawOverwrite(curr_x + 4, curr_y + 4, cardSprites, inf.power);
  curr_y += 8;
  curr_x = x + 16;
  //
  ...
  for (char i = 0; i < 4; i++) {
    sprites.drawOverwrite(curr_x += 8, curr_y, cardSprites, 2);
    sprites.drawOverwrite(curr_x += 8, curr_y, cardSprites, 0);
    sprites.drawOverwrite(curr_x += 8, curr_y, cardSprites, 0);
    sprites.drawOverwrite(curr_x += 8, curr_y, cardSprites, 0);
    sprites.drawOverwrite(curr_x += 8, curr_y, cardSprites, 0);
    sprites.drawOverwrite(curr_x += 8, curr_y, cardSprites, 0);
    sprites.drawOverwrite(curr_x += 8, curr_y, cardSprites, 3);
    curr_y += 8;
    curr_x = x;
  }
...
void stateShowStartRound() {
  ...
  if (game_mode == GAME_MODE_ADVANCED) {
    ...
    sprites.drawOverwrite(96, 8, cardSprites, current_suit + 10);
...
void stateShowPlayer1Hand () {
  ...
  else if (arduboy.justPressed(RIGHT_BUTTON) && (hand_ptr < player_p_hand_size)) hand_ptr++;
  else if (arduboy.justPressed(LEFT_BUTTON) && (hand_ptr > 0)) hand_ptr--;
  else if (arduboy.justPressed(A_BUTTON | B_BUTTON)) {
...
void stateShowPlayer2Hand () {
  ...
  else if (arduboy.justPressed(RIGHT_BUTTON) && (hand_ptr < player_c_hand_size)) hand_ptr++;
  else if (arduboy.justPressed(LEFT_BUTTON) && (hand_ptr > 0)) hand_ptr--;
  else if (arduboy.justPressed(A_BUTTON | B_BUTTON)) {
...
void stateShowComputerCardPlayed() {
  ...
  if (arduboy.justPressed(UP_BUTTON)) disp_state = GAME_SHOW_REALLY_QUIT;
  else if (arduboy.justPressed(A_BUTTON | B_BUTTON)) {
    disp_state = GAME_START_PLAYER_1_HAND;
  }
}
...
const FunctionPointer PROGMEM gamePlayingLoop[] = {
  startPlayRound,
  stateShowStartRound,
  stateShowDrawCard,
...
void stateGamePlaying() {
  ((FunctionPointer) pgm_read_word (&gamePlayingLoop[disp_state]))();
};
